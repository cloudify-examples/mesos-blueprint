          ...
          (do
            ...
            (let
              [ hostcnt (count @hostmap)
                metricavg (/ (apply + (map (fn [a] (val a)) @hostmap)) hostcnt)
                cooling (not (nil? (riemann.index/lookup index "scaling" "suspended")))]

              (do
                (info "cooling=" cooling " scale_direction={{scale_direction}} hostcnt=" hostcnt " scale_threshold={{scale_threshold}} metricavg=" metricavg)
                (if (and (not cooling) ({{scale_direction}} hostcnt {{scale_limit}}) ({{scale_direction}} {{scale_threshold}} metricavg))
                  (do
